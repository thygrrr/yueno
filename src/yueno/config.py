...
class Paths:
    ...
    build_dir: Path
    exe: Path
    models: Path  # root holding one directory per model package (e.g. models/Yue2-3B-GGUF)
    outputs: Path
    ...
    def resolve(cls) -> "Paths":
        ...
        models = Path(models_env).expanduser().resolve() if models_env else root / "models"
